 ...
 *
 * Creates a branded Google Slides deck from report data and shares it
 * with the MTM Google account. Returns the presentation URL.
 *
 * Environment Variables Required:
 *   GOOGLE_SERVICE_ACCOUNT  — Full service account JSON (already set from Tool 02)
 *   MTM_GOOGLE_EMAIL        — MTM Google account email to share the deck with
 *
 * Accepts:  { report: { client, period, ghl, ga4, takeaways, generatedAt } }
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!env.MTM_GOOGLE_EMAIL) {
      return new Response(
        JSON.stringify({ error: 'MTM_GOOGLE_EMAIL not configured. Add your Google account email to env vars.' }),
        { status: 500, headers }
      );
    }

    // Get Google access token with Slides + Drive scopes
...
async function sharePresentation(presentationId, email, token) {
  const resp = await fetch(
    `${DRIVE_BASE}/files/${presentationId}/permissions?sendNotificationEmail=false`,
    {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  if (data.error) {
    console.error('Drive share error:', JSON.stringify(data.error));
    // Non-fatal — deck was still created
  }
}
