...

{
  ...
  "recommendedApproach": {
    ...
    // Parse JSON — handle markdown fences, preamble text, and partial responses
    let brief;
    try {
      ...
      try {
        ...
        // Strategy 2: find the JSON block by locating first { and last }
        ...
        if (start !== -1 && end !== -1 && end > start) {
          try {
            ...
            // Strategy 3: if JSON is truncated, try to find last complete top-level field
            // and close the JSON gracefully enough to extract what we have
            const partial = cleaned.slice(start, end + 1);
            // Try adding closing braces in case of truncation
